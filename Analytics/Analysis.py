...
class ExamThreeGradeAnalysis:
    def __init__(self, BaseData, xcol, ycol='ExamThreeGrade'):
    ...
    def PlotBox(self):
        return sns.boxplot(self.FilteredData, x=self.xcol, y=self.ycol, hue=self.xcol, palette='Set1')

    def PlotStrip(self):
        return sns.stripplot(self.FilteredData, x=self.xcol, y=self.ycol, hue=self.xcol, palette='Set1')
    ...
    def PlotMeanAsBar(self):
        barplot = sns.barplot(x=self.MeanByCategory['Category'], y=self.MeanByCategory['Mean'], hue=self.MeanByCategory['Category'], palette='Set1')
